...
Intended usage:
    ...
    spdb.set_trace()
    ```/
Support for the full pdb functionality is not provided at this time.
...
script it synchronizes.  The file has the following format.  For each chunk of
code extracted from a document for execution, the file contains a line with 
the following information:
    ```
    <code lineno>, <doc name>, <doc lineno>, <chunk length>
    ```/
The format is thus a comma-separated value (csv) format.  The elements are 
defined as follows:
  * <code lineno>:  The line of the executed code on which the current chunk 
    of code is inserted.
  * <doc name>:  The name of the document file from which the current chunk of 
    code was extracted.  This should be quoted if it contains commas.  It may 
    be left blank if this is the main document file or the only file from 
    which code is extracted; it is only necessary if its absence would leave
    any ambiguity.
  * <doc lineno>:  The line number in the document file from which the chunk 
    of code was extracted.
...
As an example, suppose that a document main.tex contains 10 lines of code,
beginning on line 50, that are to be executed.  When this code is inserted 
into a template for execution, it begins on line 75 of the code that is 
actually executed.  In this case, the .syncdb file would contain the following
information for this chunk of code.
    ```
    75, , 50, 10
    ```/
The name of the file is omitted since it is the main file in the project.
Suppose, however, that the code was located on line 5 of a file input.tex that
is brought into the main.tex file.  In that case, the .syncdb would contain
    ```
    75, input.tex, 5, 10
    ```/
...
with variable replacement.  In such cases, the `<doc name>, <doc lineno>,` 
may be repeated for each location with a link to a given line of code, 
allowing a complete traceback of the code's origin to be assembled.  The 
rightmost `<doc name>, <doc lineno>,` should be the most specific of all
such pairs.
